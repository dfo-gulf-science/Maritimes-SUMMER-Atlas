...
distribution.usingbiomass.fct <- function(catch.df) {
  ...
  # catch.df <- read.csv(file.path(figdata.path, "/SS13_catch.csv"))
  #yrs<- seq(min(catch.df$YEAR),max(catch.df$YEAR))
  
  # 440:466,470:478,480:485:,490:495
  ...
  non.zero.catch <- subset(catch.df, totwgt != 0)
  ...
  #my.prop.df <- tidyr::pivot_longer(my.prop.df, cols=c("Year","Var2","Freq"), names_to="stratum", values_to="prop")
...
", sep = "")
  ...
  merged.df$area.occupied <-
    merged.df$prop * merged.df$AREA  * (1.852 ^ 2)
  
  area.occ <-
    tapply(na.omit(merged.df)$area.occupied,
           na.omit(merged.df)$year,
           sum)
  
  # Gini index
  summer.strata.stats.df <- subset(strata.stats.df,
  ...
  # )
  summer.strata.tt <-
    summer.strata.stats.df[order(summer.strata.stats.df$STRAT), ]
  summer.strat <-
    data.frame(
      Strata = summer.strata.tt$STRAT,
      area = summer.strata.tt$AREA,
      NH = summer.strata.tt$AREA / 0.011800615
    )
  ...
  stratified.mat <-
    lapply(yrs, function(i) {
      tapply(
        subset(merged.catch, YEAR == i)$totwgt.corr,
        subset(merged.catch, YEAR == i)$Strata,
        mean
      )
    })
  ...
  stratified.weighted.mat <- lapply(ll, function(i) {
    ...
    ss.tt <-
      subset(summer.strat, Strata %in% names(stratified.mat[[i]]))
  ...
  dwao.df <-
    data.frame(
      year = yrs,
      survey.area = rep(NA, length(yrs)),
      dwao = rep(NA, length(yrs))
    )
  d50.d75.d95.df <-
    data.frame(
      year = yrs,
      survey.area = rep(NA, length(yrs)),
      D50 = rep(NA, length(yrs)),
      D75 = rep(NA, length(yrs)),
      D95 = rep(NA, length(yrs))
    )
  ...
  for (i in 1:length(yrs)) {
    ...
    survey.area <-
      sum(st.area[st.area$stratum %in% ss, "area"]) * (1.852 ^ 2)
    
    z.out <-
      merge(merge(z, n.tows, by = "Strata"),
            st.area,
            by.x = "Strata",
            by.y = "stratum")
    ...
    if (DWAO > 0) {
      ...
      for (ii in 1:length(cc)) {
        ...
        res[ii, "F"] <-
          100 * (
            sum(
              z.out$area.divided.by.tows * z.out$totwgt.corr * z.out$I
            ) / sum(z.out$area.divided.by.tows * z.out$totwgt.corr)
          )
      ...
      b75.ii <-
        c(table(itv)[1] + table(itv)[2], table(itv)[1] + table(itv)[2] + 1)
      b50.ii <-
        c(
          table(itv)[1] + table(itv)[2] + table(itv)[3],
          table(itv)[1] + table(itv)[2]++table(itv)[3] + 1
        )
      
      ## linear interpolation for D75 and D95, based on where the breaks occurs
      diff.95 <-
        1 - ((res[b95.ii[2], "F"] - 5) / (res[b95.ii[2], "F"] - res[b95.ii[1], "F"])) ## percentage of the interval below 5%
      diff.75 <-
        1 - ((res[b75.ii[2], "F"] - 25) / (res[b75.ii[2], "F"] - res[b75.ii[1], "F"])) ## percentage of the interval below 25%
      diff.50 <-
        1 - ((res[b50.ii[2], "F"] - 50) / (res[b50.ii[2], "F"] - res[b50.ii[1], "F"])) ## percentage of the interval below 50%
      
      ##
      nr <- nrow(res)
      D.50 <-
        res[nr, "G"] - (res[b50.ii[1], "G"] + (diff.50 * (res[b50.ii[2], "G"] - res[b50.ii[1], "G"])))
      D.75 <-
        res[nr, "G"] - (res[b75.ii[1], "G"] + (diff.75 * (res[b75.ii[2], "G"] - res[b75.ii[1], "G"])))
      D.95 <-
        res[nr, "G"] - (res[b95.ii[1], "G"] + (diff.95 * (res[b95.ii[2], "G"] - res[b95.ii[1], "G"])))
      
      dwao.df[dwao.df$year == yrs[i], "dwao"] <- DWAO
      dwao.df[dwao.df$year == yrs[i], "survey.area"] <- survey.area
      d50.d75.d95.df[d50.d75.d95.df$year == yrs[i], "survey.area"] <-
        survey.area
      d50.d75.d95.df[d50.d75.d95.df$year == yrs[i], "D50"] <-
        D.50 * (1.852 ^ 2)
      d50.d75.d95.df[d50.d75.d95.df$year == yrs[i], "D75"] <-
        D.75 * (1.852 ^ 2)
      d50.d75.d95.df[d50.d75.d95.df$year == yrs[i], "D95"] <-
        D.95 * (1.852 ^ 2)
    } else {
  ...
  final.df <- data.frame(
    year = dwao.df$year,
    area.surveyed = dwao.df$survey.area / 1000,
    DWAO = dwao.df$dwao / 1000,
    D75 = d50.d75.d95.df$D75 / 1000,
    D95 = d50.d75.d95.df$D95 / 1000
  )
