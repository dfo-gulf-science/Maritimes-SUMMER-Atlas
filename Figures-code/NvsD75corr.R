...
NvsD75corr.fct <- function(spec.code) {
  ...
  xlabel1 = "Mean abundance (N) per tow"
  ...
  if(range(b.dat.in$n)[2]<10){par(mar=c(3,5,1,1))}else{par(mar=c(3,4,1,1))}
  ...
  ll <- dim(merged.df)[1]
  sapply(1:(ll-1), function(i){segments(merged.df$n[i], merged.df$D75[i], merged.df$n[i+1], merged.df$D75[i+1], col=yr.cols[i], lty=1, lwd=1)})
