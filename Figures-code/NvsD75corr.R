...
figure17.fct <- function(dat.in, cex.in, pos.ylabel=c(0,0), which.measure=c('areaocc','D','Gini')) {
#abundance <- read.csv(file.path(path.ATLAS,"Data\\SS10_stratified.csv"))
#distribution <- read.csv(file.path(path.ATLAS,"Data\\SS10_distribution.csv"))
...
merged.df <- merge(abundance, distribution, "year")
	...
	xlabel = "Mean abundance / Abondance moyenne"
	xlabel1 = "Mean number per tow"
	xlabel2 = "Nombre moyen par trait"
...
plot(area.occupied~n, data=merged.df, pch=20, axes=FALSE, ann=FALSE, xlim=x.range, ylim=y.range, col=yr.cols)
...
sapply(1:(ll-1), function(i){segments(merged.df$n[i], merged.df$area.occupied[i], merged.df$n[i+1], merged.df$area.occupied[i+1], col=yr.cols[i], lty=1, lwd=0.15)})


pearson.corr <- round(cor(merged.df$area.occupied, merged.df$n),3)
xx<-pretty.x[2]
yy<-pretty.y[6]
...
## D75
	...
	plot(D75~n, data=merged.df, pch=20, axes=FALSE, ann=FALSE, xlim=x.range, ylim=y.range, col=yr.cols)
	ll <- dim(merged.df)[1]
	sapply(1:(ll-1), function(i){segments(merged.df$n[i], merged.df$D75[i], merged.df$n[i+1], merged.df$D75[i+1], col=yr.cols[i], lty=1, lwd=0.15)})
	
	my.cor <- cor.test(merged.df$D75, merged.df$n)
...
plot(Gini~n, data=merged.df, pch=20, axes=FALSE, ann=FALSE, xlim=x.range, ylim=y.range, col=yr.cols)
ll <- dim(merged.df)[1]
sapply(1:(ll-1), function(i){segments(merged.df$n[i], merged.df$Gini[i], merged.df$n[i+1], merged.df$Gini[i+1], col=yr.cols[i], lty=1, lwd=0.15)})

pearson.corr <- round(cor(merged.df$Gini, merged.df$n),3)
xx<-pretty.x[2]
yy<-pretty.y[6]
text(xx,yy,paste("Pearson = ",pearson.corr,sep=""),cex=1.0)
