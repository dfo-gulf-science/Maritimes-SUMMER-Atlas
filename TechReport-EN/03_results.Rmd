...
The figures generated for each species are presented in Section \@ref(app:appendix).
...
\begin{landscapepage}
```{r map3, fig.cap="(ref:caption-map3)", out.width="100%"}
...
tows.strata.year <- read.csv("Atlas-summary-table-tows-by-year-stratum.csv")
tows.strata.year$Area <- round(tows.strata.year$Area) ## round each stratum area
...
csas_table(tows.strata.year.1,
  ...
  longtable = TRUE, format = "latex", align=c("c",rep("r",15))
  ) %>%
...
```{r tabtowstratumyear4, results="asis"}
yrs <- 1997:2005
ii <- which(substring(names(tows.strata.year),3,6) %in% as.character(yrs))
tows.strata.year.2 <- tows.strata.year[,c(1,2,3,ii)]
nr <- nrow(tows.strata.year.2)
...
```{r tabtowstratumyear5, results="asis"}
yrs <- 2006:2014
ii <- which(substring(names(tows.strata.year),3,6) %in% as.character(yrs))
tows.strata.year.2 <- tows.strata.year[,c(1,2,3,ii)]
nr <- nrow(tows.strata.year.2)
...
```{r tabtowstratumyear6, results="asis"}
yrs <- 2015:2020
ii <- which(substring(names(tows.strata.year),3,6) %in% as.character(yrs))
